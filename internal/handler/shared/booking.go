...
func (h *Handler) ListBookings(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		utilities.WriteJSON(w, http.StatusInternalServerError, map[string]any{"error": "failed to list bookings"})
		return
	}
...
func (h *Handler) CreateBooking(w http.ResponseWriter, r *http.Request) {
	...
	utilities.WriteJSON(w, http.StatusCreated, map[string]any{"message": "use POST /interviews/schedule to create bookings"})
}
...
func (h *Handler) UpdateBooking(w http.ResponseWriter, r *http.Request) {
	...
	utilities.WriteJSON(w, http.StatusOK, map[string]any{"id": tsID, "updated": true, "source": "qs"})
}
...
func (h *Handler) UpdateBookingReward(w http.ResponseWriter, r *http.Request) {
	...
	bookingID, err := utilities.ParseIDParam(r, "id")
	if err != nil {
		utilities.WriteJSON(w, http.StatusBadRequest, map[string]any{"error": err.Error()})
		return
	}
	...
	if err := h.BookingService.UpsertReward(ctx, bookingID, req.RewardPoints, req.RewardStatus); err != nil {
		...
		utilities.WriteJSON(w, http.StatusInternalServerError, map[string]any{"error": "failed to update reward"})
		return
	}
	utilities.WriteJSON(w, http.StatusOK, map[string]any{
		"id":           bookingID,
