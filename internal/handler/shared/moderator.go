...
func (h *Handler) ListModerators(w http.ResponseWriter, r *http.Request) {
	...
	if h.QsUserRepo != nil {
		...
		if err != nil {
			...
			for _, m := range mods {
				roles := []string{}
				for _, rid := range core.ParseRoleCSV(m.RoleIDs) {
					roles = append(roles, core.QsRoleName(rid))
				}
	...
	core.WriteJSON(w, http.StatusOK, combined)
}
...
func (h *Handler) CreateModerator(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		core.WriteJSON(w, http.StatusInternalServerError, map[string]any{"error": "failed to create moderator"})
		return
	}
...
func (h *Handler) GetModerator(w http.ResponseWriter, r *http.Request) {
	...
	case "qs":
		...
		if err != nil {
			...
			core.WriteJSON(w, http.StatusNotFound, map[string]any{"error": "moderator not found"})
			return
		}
		roles := []string{}
	...
	case "iris":
		if h.IrisUserRepo == nil {
			core.WriteJSON(w, http.StatusServiceUnavailable, map[string]any{"error": "IRIS database unavailable"})
			return
		}
	...
	default:
		core.WriteJSON(w, http.StatusBadRequest, map[string]any{"error": "invalid source, use qs or iris"})
	}
}
...
func (h *Handler) UpdateModerator(w http.ResponseWriter, r *http.Request) {
	...
	default:
		core.WriteJSON(w, http.StatusBadRequest, map[string]any{"error": "invalid source"})
		return
	}

	core.WriteJSON(w, http.StatusOK, map[string]any{"updated": true, "id": nid})
}
...
func (h *Handler) DeleteModerator(w http.ResponseWriter, r *http.Request) {
	...
	core.WriteJSON(w, http.StatusOK, map[string]any{"numberOfRecordsUpdated": 1})
}
...
func (h *Handler) GetModeratorTimeslots(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		core.WriteJSON(w, http.StatusInternalServerError, map[string]any{"error": "failed to get moderator timeslots"})
		return
	}
...
func (h *Handler) GetModeratorAvailability(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		core.WriteJSON(w, http.StatusInternalServerError, map[string]any{"error": "failed to fetch availability"})
		return
	}
	...
	core.WriteJSON(w, http.StatusOK, map[string]any{"availabilities": items})
}
...
func (h *Handler) PostModeratorAvailability(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		core.WriteJSON(w, http.StatusBadRequest, map[string]any{"error": "invalid startTime format, use RFC3339"})
		return
	}
	et, err := time.Parse(time.RFC3339, body.EndTime)
	if err != nil {
		core.WriteJSON(w, http.StatusBadRequest, map[string]any{"error": "invalid endTime format, use RFC3339"})
		return
	}
	...
	if err != nil {
		...
		core.WriteJSON(w, http.StatusInternalServerError, map[string]any{"error": "failed to create availability"})
		return
	}
...
func (h *Handler) DeleteModeratorAvailability(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.QsUserRepo.DeleteModeratorAvailability(ctx, nid); err != nil {
		...
		core.WriteJSON(w, http.StatusInternalServerError, map[string]any{"error": "failed to delete availability"})
		return
	}

	core.WriteJSON(w, http.StatusOK, map[string]any{"deleted": true, "availabilityId": nid})
}
