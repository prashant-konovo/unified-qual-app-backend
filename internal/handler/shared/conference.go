...
func (h *Handler) ConferenceLogin(w http.ResponseWriter, r *http.Request) {
	confHashStr, _ := utilities.ParseStringParam(r, "confId")
	...
	if h.ConferenceService.Available() {
		...
		if err != nil {
			if strings.Contains(err.Error(), "invalid pin") {
				utilities.WriteJSON(w, http.StatusUnauthorized, map[string]any{"error": "invalid pin"})
				return
			}
			...
			utilities.WriteJSON(w, http.StatusInternalServerError, map[string]any{"error": "login failed"})
			return
		}
...
func (h *Handler) GetConferenceParticipants(w http.ResponseWriter, r *http.Request) {
	...
	utilities.WriteJSON(w, http.StatusOK, map[string]any{"participants": []any{}, "timeSlot": nil})
}
...
func (h *Handler) GetMeetingMetadata(w http.ResponseWriter, r *http.Request) {
	...
	bearerToken := utilities.ExtractBearerToken(r)
	...
	if hash == "" && h.ConferenceService.ConferenceConfigured() {
		...
		if err == nil && meta != nil {
			utilities.WriteJSON(w, http.StatusOK, meta)
			return
		}
	}

	if hash == "" {
		utilities.WriteJSON(w, http.StatusBadRequest, map[string]any{"error": "hash parameter required"})
		return
	}
	...
	if h.ConferenceService.Available() {
		...
		if meta == nil {
			utilities.WriteJSON(w, http.StatusNotFound, map[string]any{"error": "meeting not found"})
			return
		}
		utilities.WriteJSON(w, http.StatusOK, meta)
		return
...
func (h *Handler) MeetingJoin(w http.ResponseWriter, r *http.Request) {
	joinID, err := utilities.ParseStringParam(r, "joinId")
	if err != nil {
...
func (h *Handler) GetAttendeesByMeetingID(w http.ResponseWriter, r *http.Request) {
	meetingID, _ := utilities.ParseStringParam(r, "meetingId")
	bearerToken := utilities.ExtractBearerToken(r)
	...
	if h.ConferenceService.ConferenceConfigured() {
		...
		if err == nil && attendees != nil {
			utilities.WriteJSON(w, http.StatusOK, attendees)
			return
		}
...
func (h *Handler) CreateMeeting(w http.ResponseWriter, r *http.Request) {
	...
	if h.ConferenceService.ConferenceConfigured() {
		...
		if err != nil {
			...
			utilities.WriteJSON(w, http.StatusBadGateway, map[string]any{"error": "failed to create meeting: " + err.Error()})
			return
		}
	...
	utilities.WriteJSON(w, http.StatusServiceUnavailable, map[string]any{"error": "conference service not configured"})
}
...
func (h *Handler) CreateTranscriptionOrder(w http.ResponseWriter, r *http.Request) {
	...
	if h.ConferenceService.CastingWordsConfigured() {
		...
		if err != nil {
			...
			utilities.WriteJSON(w, http.StatusBadGateway, map[string]any{"error": "transcription order failed: " + err.Error()})
			return
		}
		utilities.WriteJSON(w, http.StatusCreated, map[string]any{
	...
	utilities.WriteJSON(w, http.StatusServiceUnavailable, map[string]any{"error": "transcription service not configured"})
}
...
func (h *Handler) GetTranscriptionStatus(w http.ResponseWriter, r *http.Request) {
	...
	if h.ConferenceService.CastingWordsConfigured() {
		...
		if err != nil {
			utilities.WriteJSON(w, http.StatusBadGateway, map[string]any{"error": "failed to get status"})
			return
		}
		utilities.WriteJSON(w, http.StatusOK, order)
		return
	}
	utilities.WriteJSON(w, http.StatusServiceUnavailable, map[string]any{"error": "transcription service not configured"})
}
...
func (h *Handler) GetTranscript(w http.ResponseWriter, r *http.Request) {
	...
	if h.ConferenceService.CastingWordsConfigured() {
		...
		if err != nil {
			utilities.WriteJSON(w, http.StatusBadGateway, map[string]any{"error": "failed to get transcript"})
			return
		}
		utilities.WriteJSON(w, http.StatusOK, map[string]any{"orderId": orderID, "transcript": transcript})
		return
	}
	utilities.WriteJSON(w, http.StatusServiceUnavailable, map[string]any{"error": "transcription service not configured"})
}
...
func (h *Handler) SendSMS(w http.ResponseWriter, r *http.Request) {
	...
	if h.ConferenceService.SMSConfigured() {
		if err := h.ConferenceService.SendSMS(r.Context(), req.To, req.From, req.Message); err != nil {
			...
			utilities.WriteJSON(w, http.StatusBadGateway, map[string]any{"error": "sms send failed: " + err.Error()})
			return
		}
		utilities.WriteJSON(w, http.StatusOK, map[string]any{"sent": true, "to": req.To})
		return
	}
	utilities.WriteJSON(w, http.StatusServiceUnavailable, map[string]any{"error": "sms service not configured"})
}
...
func (h *Handler) MeetingAction(w http.ResponseWriter, r *http.Request) {
	...
	if h.ConferenceService.ConferenceConfigured() {
		...
		if err == nil && meta != nil {
			...
			meta["actionTimestamp"] = utilities.Now()
			utilities.WriteJSON(w, http.StatusOK, meta)
			return
		}
	}
	...
	utilities.WriteJSON(w, http.StatusOK, map[string]any{
		...
		"result": "success", "timestamp": utilities.Now(),
	})
}
...
func (h *Handler) MeetingUniversalJoin(w http.ResponseWriter, r *http.Request) {
	...
	if h.ConferenceService.ConferenceConfigured() {
		...
		if err == nil && joinResp != nil {
			joinResp["joinTimestamp"] = utilities.Now()
			utilities.WriteJSON(w, http.StatusOK, joinResp)
			return
		}
	...
	if h.ConferenceService.Available() {
		...
		if err == nil && meta != nil {
			...
			meta["joinTimestamp"] = utilities.Now()
			utilities.WriteJSON(w, http.StatusOK, meta)
			return
		}
	}
	...
	utilities.WriteJSON(w, http.StatusOK, map[string]any{
		...
		"attendeeId":    "att-" + utilities.ID()[:8],
		"joinTimestamp": utilities.Now(),
	})
}
