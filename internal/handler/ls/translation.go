...
func (h *Handler) DeleteTopicTranslation(w http.ResponseWriter, r *http.Request) {
	...
	if len(parts) < 2 {
		core.WriteJSON(w, http.StatusBadRequest, map[string]any{"error": "invalid translation key format: topicId_langCode"})
		return
	}
	...
	if h.QsAnswerRepo != nil {
		...
		core.WriteJSON(w, http.StatusOK, map[string]any{"deleted": true, "projectId": projectID, "topicId": topicID, "languageCode": langCode})
		return
	}
...
func (h *Handler) DeleteTranslation(w http.ResponseWriter, r *http.Request) {
	...
	if h.QsAnswerRepo != nil {
		...
		core.WriteJSON(w, http.StatusOK, map[string]any{"deleted": true, "projectId": projectID, "languageCode": langCode})
		return
	}
	core.WriteJSON(w, http.StatusServiceUnavailable, map[string]any{"error": "no database available"})
}
