...
func (h *Handler) AddHonorariumAmountMRA(w http.ResponseWriter, r *http.Request) {
	...
	if projectID == 0 {
		core.WriteJSON(w, http.StatusInternalServerError, map[string]any{"message": "missing projectId param"})
		return
	}
	if honorarium == 0 {
		core.WriteJSON(w, http.StatusInternalServerError, map[string]any{"message": "missing honorarium param"})
		return
	}
	if body.Currency == "" {
		core.WriteJSON(w, http.StatusInternalServerError, map[string]any{"message": "missing currency param"})
		return
	}
	if body.SessKey == "" {
		core.WriteJSON(w, http.StatusInternalServerError, map[string]any{"message": "missing sessKey param"})
		return
	}
	...
	core.WriteJSON(w, http.StatusOK, map[string]any{"message": "Successfully Added honorarium amount"})
}
...
func (h *Handler) AddTimeSlotPaymentsMRA(w http.ResponseWriter, r *http.Request) {
	...
	core.WriteJSON(w, http.StatusCreated, map[string]any{"message": "Success"})
}
