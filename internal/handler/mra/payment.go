...
func (h *Handler) AddHonorariumAmountMRA(w http.ResponseWriter, r *http.Request) {
	...
	if projectID == 0 {
		utilities.WriteJSON(w, http.StatusInternalServerError, map[string]any{"message": "missing projectId param"})
		return
	}
	if honorarium == 0 {
		utilities.WriteJSON(w, http.StatusInternalServerError, map[string]any{"message": "missing honorarium param"})
		return
	}
	if body.Currency == "" {
		utilities.WriteJSON(w, http.StatusInternalServerError, map[string]any{"message": "missing currency param"})
		return
	}
	if body.SessKey == "" {
		utilities.WriteJSON(w, http.StatusInternalServerError, map[string]any{"message": "missing sessKey param"})
		return
	}
	...
	utilities.WriteJSON(w, http.StatusOK, map[string]any{"message": "Successfully Added honorarium amount"})
}
...
func (h *Handler) AddTimeSlotPaymentsMRA(w http.ResponseWriter, r *http.Request) {
	...
	utilities.WriteJSON(w, http.StatusCreated, map[string]any{"message": "Success"})
}
