...
func (h *Handler) AddConferenceLinkMRA(w http.ResponseWriter, r *http.Request) {
	...
	participantGroupID, err := utilities.ParseIDParam(r, "participant_group_id")
	if err != nil {
		utilities.WriteJSON(w, http.StatusBadRequest, map[string]any{"error": err.Error()})
		return
	}
...
func (h *Handler) GetConferenceLinkMRA(w http.ResponseWriter, r *http.Request) {
	...
	projectID, err := utilities.ParseIDParam(r, "participant_group_id")
	if err != nil {
		utilities.WriteJSON(w, http.StatusBadRequest, map[string]any{"error": err.Error()})
		return
	}
	...
	utilities.WriteJSON(w, http.StatusOK, resObj)
}
...
func (h *Handler) GetConfLinkByTimeSlotMRA(w http.ResponseWriter, r *http.Request) {
	...
	tsID, err := utilities.ParseIDParam(r, "timeslot_id")
	if err != nil {
		utilities.WriteJSON(w, http.StatusBadRequest, map[string]any{"error": err.Error()})
		return
	}
