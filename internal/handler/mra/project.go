...
func (h *Handler) CreateProjectMRA(w http.ResponseWriter, r *http.Request) {
	...
	dto.WriteJSON(w, http.StatusOK, record)
}
...
func (h *Handler) ListProjectsMRA(w http.ResponseWriter, r *http.Request) {
	...
	dto.WriteJSON(w, http.StatusOK, output)
}
...
func (h *Handler) ResetProjectModeratorsMRA(w http.ResponseWriter, r *http.Request) {
	...
	if mode == "reset" {
		...
		dto.WriteJSON(w, http.StatusOK, mods)
	} else {
		dto.WriteJSON(w, 422, map[string]any{
...
func (h *Handler) HandleProjectExportMRA(w http.ResponseWriter, r *http.Request) {
	...
	if h.ProjectService.S3ExportConfigured() {
		...
		dto.WriteJSON(w, http.StatusOK, map[string]any{"fileURL": presignedURL})
		return
	}

	dto.WriteJSON(w, http.StatusOK, map[string]any{"fileURL": "", "rows": len(allRows)})
}
...
func (h *Handler) GetModeratorsCountMRA(w http.ResponseWriter, r *http.Request) {
	...
	sampleSize, err := dto.ParseIDParam(r, "sample_size")
	if err != nil {
		dto.WriteJSON(w, http.StatusBadRequest, map[string]any{"error": err.Error()})
		return
	}
