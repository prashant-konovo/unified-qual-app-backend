...
func (h *Handler) ListProjectsMRA(w http.ResponseWriter, r *http.Request) {
	...
	httpkit.WriteJSON(w, http.StatusOK, output)
}
...
func (h *Handler) ResetProjectModeratorsMRA(w http.ResponseWriter, r *http.Request) {
	...
	if mode == "reset" {
		...
		httpkit.WriteJSON(w, 422, map[string]any{
...
func (h *Handler) HandleProjectExportMRA(w http.ResponseWriter, r *http.Request) {
	...
	if h.ProjectService.S3ExportConfigured() {
		...
		httpkit.WriteJSON(w, http.StatusOK, map[string]any{"fileURL": presignedURL})
		return
	}

	httpkit.WriteJSON(w, http.StatusOK, map[string]any{"fileURL": "", "rows": len(allRows)})
}
...
func (h *Handler) GetModeratorsCountMRA(w http.ResponseWriter, r *http.Request) {
	...
	sampleSize, err := httpkit.ParseIDParam(r, "sample_size")
	if err != nil {
		httpkit.WriteJSON(w, http.StatusBadRequest, map[string]any{"error": err.Error()})
		return
	}
