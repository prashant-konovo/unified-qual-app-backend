...
func (h *Handler) DeleteTopicTranslationMRA(w http.ResponseWriter, r *http.Request) {
	...
	if !ok {
		httpkit.WriteJSON(w, http.StatusBadRequest, map[string]any{"error": "unknown language code"})
		return
	}
...
func (h *Handler) DeleteTranslationMRA(w http.ResponseWriter, r *http.Request) {
	...
	if languageID == 0 {
		httpkit.WriteJSON(w, http.StatusBadRequest, map[string]any{"error": "invalid language code"})
		return
	}
