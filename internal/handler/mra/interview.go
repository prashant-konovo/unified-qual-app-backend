...
func (h *Handler) GetAllInterviewsMRA(w http.ResponseWriter, r *http.Request) {
	...
	if h.QsInterviewsRepo == nil {
		core.WriteJSON(w, http.StatusInternalServerError, map[string]any{"error": "database not configured"})
		return
	}
	...
	if err != nil {
		...
		core.WriteJSON(w, http.StatusInternalServerError, map[string]any{"error": err.Error()})
		return
	}
	if data == nil {
		data = []map[string]any{}
...
func (h *Handler) InvalidateInterviewMRA(w http.ResponseWriter, r *http.Request) {
	...
	if len(validationErrors) > 0 {
		core.WriteJSON(w, http.StatusBadRequest, map[string]any{"error": validationErrors})
		return
	}
...
func (h *Handler) SendInvalidateRescheduleMailMRA(w http.ResponseWriter, r *http.Request) {
	...
	if body.IsIneligibleMail != nil && *body.IsIneligibleMail {
		...
		if len(participantIDs) == 0 {
			core.WriteJSON(w, http.StatusBadRequest, map[string]any{"error": "participantId is required"})
			return
		}
	...
	if timeSlotID == 0 {
		core.WriteJSON(w, http.StatusBadRequest, map[string]any{"error": "timeSlotId is required"})
		return
	}
	...
	ts, err := h.QsTimeSlotRepo.GetByID(r.Context(), timeSlotID)
	if err != nil || ts == nil {
		core.WriteJSON(w, http.StatusNotFound, map[string]any{"error": "Timeslot not found"})
		return
	}

	if !ts.IsInvalidatedInterview {
		core.WriteJSON(w, http.StatusConflict, map[string]any{"error": "Interview not invalidated yet!"})
		return
	}

	if ts.IsInvalidateEmailSent {
		core.WriteJSON(w, http.StatusConflict, map[string]any{"error": "Invalidate Reschedule Email Already Sent"})
		return
	}
	...
	core.WriteJSON(w, http.StatusOK, map[string]any{"status": "SUCCESS"})
}
...
func (h *Handler) CancelRescheduleAction(w http.ResponseWriter, r *http.Request) {
	...
	default:
		core.WriteJSON(w, http.StatusBadRequest, map[string]any{"error": "action must be cancel, reschedule, pmCancel, pmReschedule, respondentReschedule, or respondentCancel"})
		return
	}
	...
	if alreadyActioned[ts.StatusID] {
		core.WriteJSON(w, http.StatusOK, core.BuildTimeSlotResponse(ts))
		return
	}
	...
	core.WriteJSON(w, http.StatusOK, core.BuildTimeSlotResponse(tsUpdated))
}
